...
(`@capacitor/local-notifications`) и они приходят при закрытом приложении; на десктопе планировщика
для закрытого приложения нет, поэтому напоминания появляются при запуске и пока окно открыто.

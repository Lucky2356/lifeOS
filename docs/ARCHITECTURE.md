...
| ------------------ | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Предметные правила | `packages/domain/src`            | схемы, вычисления, инварианты: `life-object`, `object-fields`, `decision`, `reminders`, `content`, `household*`, `attachment` |
...
### Контент-пак и прогресс

Пак приезжает вместе с релизом приложения, поэтому сохранённый прогресс может относиться к прежнему
набору шагов. Набор ключей задаёт плейбук, а не запись: `reconcileProgress` (домен) сохраняет
отметки уцелевших шагов, забывает исчезнувшие и приводит добавленный шаг неотмеченным.
Согласование происходит на выходе из `navigatorStore` — выше по стеку никто не обязан помнить, что
контент меняется. Прогресс по плейбуку, которого в паке больше нет, из выдачи исключается, но из
базы не удаляется: контент может вернуться, а молча стирать отметки пользователя нельзя.

Ссылочную целостность пака проверяет схема, а не скрипт: `embedsGuideKey` обязан указывать на
существующий плейбук вида `bureaucracy`, ключи уникальны, шаги пронумерованы подряд, переводы
непусты на обоих языках. Скрипт `validate:content` только зовёт схему — так же поступит любая
будущая проверка пака в рантайме.

Тексты процедур пишутся без точных сумм, номеров статей и календарных сроков: это административные
процедуры, они меняются, а проверить их из репозитория нечем. Где срок принципиален, шаг
формулирует его относительно события и отправляет к первоисточнику.

## Напоминания

Пороги считает домен (`computeReminders` + `defaultReminderRules`). Наступившие показываются сразу и
запоминаются в `settings`, чтобы не повторяться; будущие планируются заранее. Домашние задачи со
сроком идут через тот же механизм, но с одним порогом — в день срока: предупреждать о выносе мусора
за 90 дней бессмысленно.
...
`apps/app/src/lib/backup.ts`; по желанию — зашифрованный паролем (`backup-crypto.ts`). Импорт
валидирует файл доменными схемами и заменяет содержимое базы целиком. Куда положить файл, решает
платформа (`platform-files.ts`): на десктопе — папка «Загрузки», на Android — системное «Поделиться»,
в браузере — обычная загрузка.

Перед импортом прежнее состояние откладывается целиком (`stashRollback`) и семь дней доступно к
возврату. Снимок живёт в хранилище `settings` не случайно: ни импорт, ни «удалить все данные» его не
чистят, поэтому он переживает и подмену базы, и перезагрузку страницы, которой импорт заканчивается.
...
Иконки — инлайновые SVG (`components/Icon.tsx` + сгенерированный `components/icon-paths.ts`), а не
веб-шрифт: шрифт Tabler весил 4,42 МБ в трёх форматах и 5807 правил CSS ради 34 иконок — около 80%
сборки. Обновляются командой `node scripts/build-icons.mjs`; список нужных имён держится в самом
скрипте, и он же падает, если в разметке появилась иконка, которой нет в списке.

## Устойчивость

Всё приложение стоит на IndexedDB, поэтому её отказ обрабатывается явно: `ErrorBoundary` вокруг
приложения ловит падение, а экраны «Сегодня» и «Реестр» показывают понятное сообщение вместо вечной
«Загрузки». Экран ошибки не предлагает очистить данные — при сбое чтения они могут быть целы.

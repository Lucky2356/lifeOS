...
| `deletedAt`               | ISO-8601 \| null | корзина: объект удалён, но 30 дней подлежит восстановлению         |
...
- Удаление объекта кладёт его в корзину и вложения **не трогает**: документ без скана —
  уже не тот документ, ради которого его восстанавливали. Каскад срабатывает только при
  окончательном удалении — вручную из корзины или автоматически через `trashRetentionDays` (30 дней,
  чистится при запуске приложения).
...
Импорт разбирает файл в два приёма. Сначала опознаётся конверт — копия ли это Life OS
(`app`) и какого поколения формата (`schema`); затем читатель своего поколения проверяет содержимое
доменными схемами. Исходов три, и они различимы: чужой файл, копия из более новой версии
приложения, своя копия с повреждением (сообщение называет путь до него). Появится `schema` 2 —
рядом появится второй читатель, а первый останется: копии, сделанные сегодня, обязаны открываться и
через год.

Содержимое вложений проверяется как base64 и раскодируется **до** открытия транзакции: внутри неё
падать нельзя, иначе уже выданная очистка успела бы закоммититься, а запись — нет. Метаданные
вложения без байтов в копию не попадают — копия не должна обещать того, чего в ней нет.

Снимок отката (`pre-import-rollback` в `settings`) проходит того же читателя, что и файл с диска:
он сделан прежней версией приложения, и его поколение так же поднимается до текущего.
...
- **Схема IndexedDB**: `schemaSteps` в `db.ts` — нумерованный список шагов, индекс + 1 равен версии,
  до которой шаг поднимает базу. `DB_VERSION` не пишется руками, а равна длине списка. `upgrade`
  начинает цикл с `oldVersion`, поэтому применённые шаги не повторяются, и передаёт шагу транзакцию
  версии: без неё нельзя ни добавить индекс к существующему хранилищу, ни переписать уже лежащие
  записи. Форму схемы фиксирует `db-migration.test.ts` — миграция, забывшая создать хранилище или
  индекс, роняет тест, а не данные.

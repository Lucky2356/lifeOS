...
Дальше `.github/workflows/release.yml` создаёт релиз, собирает установщик Windows (подписывая
артефакты апдейтера) и релизный APK, проверяет подпись APK и прикрепляет всё к релизу.
